...
class BaseApplication
{
  ...
  private:
    ...
    void CreateInstance()
    {
        ...
        // THIS IS THE VULKAN API NOT JUST A RANDOM NUMBER IVE GIVEN. (tutorial
        // was 1.0 so im avoiding using 1.3)
        ...
        // Thank you vulkan for having actual fucking function names not
        // DXGI11_CREATE_FACTORY bullshit
    ...
    bool IsPhysicalDeviceSuitable(VkPhysicalDevice a_device)
    {
        ...
        // we only want discrete gpus and if they are capable of geometry shaders.
        // and it can support queues that take graphics commands.
        ...
        // Small note. as the tutorial mentions you can rank physical devices by
        // score and use a std::multimap<int, VkPhysicalDevice> to auto sort by
        // highest score. didnt know multimap was a thing that is very cool.
    }
    struct QueueFamilyIndicies
    {
        // using optional since uint32_t will always be a value but with optional it
        // will be blank until we have a value for it
    ...
    {
        glfwInit();
        // Glfw will default to using opengl, so we specify that this is not the
        // case.
        glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);
        // resizeable windows require more management so for now it will be
        // disabled.
        glfwWindowHint(GLFW_RESIZABLE, GLFW_FALSE);

        // monitor = what monitor you want to launch the program on, share pointer
        // is just for opengl.
...
#pragma region Debugging
    VkResult CreateDebugUtilsMessengerEXT(VkInstance a_instance, const VkDebugUtilsMessengerCreateInfoEXT* pCreateInfo, VkAllocationCallbacks* pAllocator, VkDebugUtilsMessengerEXT* pDebugMessenger)
    {
    ...
    void SetupDebugManager()
    {
        ...
        // Void pointer for pUserData so we could gather everything about the users
        // pc and give it to the callback for instance.
    ...
    // VKAPI_CALL= __stdcall on windows (just tells the stack how to use the
    // function. VKAPI_ATTR = nothing on windows so we are actually just doing
